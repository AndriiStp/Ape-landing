...
import { apeItems } from './apeCollection';
      ...
      >
        {apeItems.map((item, index) => (
          <SwiperSlide className={css.slideItem} key={index}>
            <picture>
              <source
                srcSet={`${item.apeBig1x} 1x, ${item.apeBig2x} 2x`}
              ...
              />
              <source
                srcSet={`${item.apeMob1x} 1x, ${item.apeMob2x} 2x`}
                media="(max-width: 480px)"
              />
              <img src={item.apeMob1x} alt={item.alt} loading="lazy" />
            </picture>
          </SwiperSlide>
        ))}
